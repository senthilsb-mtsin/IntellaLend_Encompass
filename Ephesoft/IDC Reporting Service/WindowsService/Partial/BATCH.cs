...
namespace MTSIDCReportService
{
    public partial class BATCH
    {
        ...
        private void LoadXMLFile(List<string> PageValuePL, List<string> FieldValuePL)
        {
            ...
            Logger.WriteTraceLog($"LoadXML File Path : {str1}");
            Logger.WriteTraceLog($"PATTERNS : {PATTERNS}");
            foreach (string format in PATTERNS.Split(';'))
            {
                ...
                Logger.WriteTraceLog($"XML File Path : {str2}");
                if (File.Exists(str2))
                {
                    ...
                    Logger.WriteTraceLog($"XML File Exsits : {str2}");
                }
                else
                    Logger.WriteTraceLog($"XML File Not Found : {str2}");
            }
            Logger.WriteTraceLog($"LoadXML File Path : {str1}");
        }
            ...
            using (StringReader stringReader = new StringReader(RootElement.ToString()))
            {
                ...
                while (xpathNodeIterator1.MoveNext())
                {
                    ...
                    while (xpathNodeIterator2.MoveNext())
                    {
                        ...
                        Service.ReportDb.PAGES.Add(entity);
                    }
                }
            }
        }
        ...
        private void SetFieldInformation(XElement RootElement, string nPattern)
        {
            using (StringReader stringReader = new StringReader(RootElement.ToString()))
            {
                ...
                while (xpathNodeIterator1.MoveNext())
                {
                    ...
                    while (xpathNodeIterator2.MoveNext())
                        Service.ReportDb.FIELDS.Add(new FIELD()
                        {

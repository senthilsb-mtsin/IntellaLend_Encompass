...
public class ScriptTableCellValueChange implements IJDomScript {

	private static EphesoftLogger LOGGER = ScriptLoggerFactory.getLogger(ScriptTableCellValueChange.class);
	...
	public Object execute(Document document, String tableName, String docIdentifier, String columnName, int rowIndex) {
		Exception exception = null;
		try {
			changeTableCell(document, tableName, docIdentifier);
		} catch (Exception e) {
	...
	public void changeTableCell(Document document, String tableName, String docIdentifier) {
		LOGGER.info("*************  Inside Table cell value change script.");

		LOGGER.info("*************  Start execution of Table cell value change  script.");

		if (null == document) {
			LOGGER.error("Input document is null.");
		}
		// Write the document object to the xml file. Currently following method call for performance improvement.
		// writeToXML(document);
		LOGGER.info("*************  End execution of the Table cell Value Change script.");
	}

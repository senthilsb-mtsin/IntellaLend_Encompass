...
public class ScriptAutomaticValidation extends BaseScript {
	...
	private static final String REST_SERVICE_URL = "http://34.224.49.101:8010/EphesoftUtilityAPI/FieldMapping/SetEncompassLoanType";
	private static final String AUTO_VALIDATION_URL = "http://34.224.49.101:8010/EphesoftUtilityAPI/AutoValidation/Execute";
	...
	public Object execute(Document document, String methodName, String documentIdentifier) {
		...
		try {
			...
			//Prakash : No LookUp for Republic Bank
	...
	private void OrderBYStacking(){
		...
		String REST_SERVICE_URL = "http://34.224.49.101:8010/EphesoftUtilityAPI/Document/OrderByStacking";		
		try {
		...
		try
		{
			...
			{	
				...
				*/
				...
				if (termsValue != null) {
					if (termsValue.getText().length() > 0) {
				...
				if (balValue != null) {
					if (balAmount.equals("")) {
						if ((!balValue.getTextTrim().equals("")) && (skipRPTD == 0)) {
	...
	public void manageCRChronosFormat(Element table)
	{
		try
		{
			...
			{	
				...
				if ((rowDEL.equals("Y")) || (balAmount.equals("$0")) || (balAmount.equals("-0-")))
				{					
					row.addContent(new Element("ToDelete").setText("DEL"));
				}
				else //if ((rowDEL.equals("N")) && (acctNoCount > 0) && (acctNameCount > 0)) 
		...
		{			
			...
			if (tables != null)
			{
				for (Element table : tables)
				{
					...
					if ((tableName.equals("Trade Info")) || (tableName.equals("Liability Table")) || (tableName.equals("Assets Table")) ||
						(tableName.equals("Monthly Income")) || (tableName.equals("Housing Expense Information")) || 
						(tableName.equals("CRTextFormat")) || (tableName.equals("CRFactualData")))
					{

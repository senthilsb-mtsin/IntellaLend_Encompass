...
namespace MTSServiceController
{
    ...
    public partial class MTSServiceController : Form
    {
        ...
        private void SetMinioParameters()
        {
            ...
            DataTable configTemp = DataAccess.ExecuteDataTable("[IL].GETSERVICECONFIG", null);
        ...
        private void ReadServiceConfigFromDB()
        {
            try
            {
                dtService = DataAccess.ExecuteDataTable("[IL].GETSERVICECONFIG", null);
            ...
            try
            {
                if (dtModified != null)
                {
                    foreach (DataRow dr in dtModified.Rows)
                    {
                        ...
                        DataAccess.ExecuteNonQueryCMD("[IL].UpdateServiceConfig", dr["SERVICENAME"], dr["SERVICEDISPLAYNAME"], dr["SERVICEDESCRIPTION"], intInvokeType, dr["SERVICEPARAMS"], dr["TIME"]);
                    }
                }
            }
            catch (Exception ex)
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            //    try
            //    {
            //        DataAccess.ExecuteNonQuery("[IL].[UPDATETIMERCOUNT]", DateTime.Now);
            //    }
            //    catch (Exception ex)
            //    {
            //        txtLog.AppendText(ex.ToString());

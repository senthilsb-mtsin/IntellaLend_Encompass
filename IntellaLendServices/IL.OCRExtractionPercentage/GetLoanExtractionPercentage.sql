...
DECLARE @BATCHCLASSCONFIGID BIGINT = 0;
DECLARE @BATCHCLASSNAME NVARCHAR(10) = '';
DECLARE @EPHESOFT_DOC_FIELD TABLE (
	DOCUMENT_NAME NVARCHAR(510)
	,FIELD_NAME NVARCHAR(510)
	);

SELECT @BATCHCLASSID = e.id
	,@BATCHCLASSNAME = e.identifier
FROM BATCHES b
INNER JOIN Ephesoft.DBO.BATCH_CLASS e ON b.batchclass_id = e.identifier
WHERE BATCH_INSTANCEID = @BATCHINSTANCEID

SELECT @BATCHCLASSCONFIGID = CONFIGID
FROM EphesoftUtility2020.DBO.[MTS.IDC_RULE_CONFIG]
WHERE EphesoftBatchClass = @BATCHCLASSNAME;

INSERT INTO @EPHESOFT_DOC_FIELD
SELECT dm.document_type_name
	,field_type_name
FROM Ephesoft.dbo.batch_class_document_type d
INNER JOIN ephesoft.dbo.field_type f ON d.document_type_id = f.document_type_id
INNER JOIN ephesoft.dbo.document_type dm ON d.document_type_id = dm.id
WHERE f.is_hidden = 0
	AND batch_class_id = @BATCHCLASSID
...
BEGIN
	...
	AS (
		...
		INNER JOIN @EPHESOFT_DOC_FIELD EP ON E.DOCTYPE = EP.DOCUMENT_NAME
			AND E.FIELDNAME = EP.FIELD_NAME
		INNER JOIN EphesoftUtility2020.dbo.MTS_AUTO_VALIDATION_SKIP IDC WITH (NOLOCK) ON IDC.ConfigId = @BATCHCLASSCONFIGID
			AND IDC.DOCUMENT_NAME = E.DOCTYPE

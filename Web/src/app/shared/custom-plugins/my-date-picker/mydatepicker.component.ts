...
export class MyDatePickerComponent implements OnChanges, ControlValueAccessor, OnDestroy {
    ...
    weekDays: string[] = [];
    dates: IMyWeek[] = [];
    months: IMyCalendarMonth[][] = [];
    years: IMyCalendarYear[][] = [];
    selectionDayTxt = '';
    invalidDate = false;
    ...
    opts: IMyOptions = {
        ...
        selectorHeight: <string> '280px',
        selectorWidth: <string> '290px',
    ...
    generateYears(year: number): void {
        ...
        for (let i = year; i <= 20 + year; i += 5) {
            const row: IMyCalendarYear[] = [];
            for (let j = i; j < i + 5; j++) {
                const disabled: boolean = this.utilService.isMonthDisabledByDisableUntil({year: j, month: this.visibleMonth.monthNbr, day: this.daysInMonth(this.visibleMonth.monthNbr, j)}, this.opts.disableUntil)
    ...
    onUserDateInput(value: string): void {
        if (value.length === 0) {
            ...
            const date: IMyDate = this.utilService.isDateValid(value, this.opts.dateFormat, this.opts.minYear, this.opts.maxYear, this.opts.disableUntil, this.opts.disableSince, this.opts.disableWeekends, this.opts.disableWeekdays, this.opts.disableDays, this.opts.disableDateRanges, this.opts.monthLabels, this.opts.enableDays);
    ...
    parseOptions(): void {
        ...
        this.setOptions();
        const weekDays: string[] = this.utilService.getWeekDays();
    ...
    writeValue(value: any): void {
        if (value && (value['date'] || value['jsdate'] || value['formatted'])) {
            ...
            const cvc: boolean = this.visibleMonth.year !== this.selectedDate.year || this.visibleMonth.monthNbr !== this.selectedDate.month;
            if (cvc) {
    ...
    ngOnChanges(changes: SimpleChanges): void {
        if (changes.hasOwnProperty('selector')) {
            const s: any = changes['selector'].currentValue;
            if (typeof s === 'object') {
                if (s.open) {
    ...
    updateDateValue(date: IMyDate): void {
        const clear: boolean = !this.utilService.isInitializedDate(date);
    ...
    parseSelectedDate(selDate: any): IMyDate {
        ...
        if (typeof selDate === 'string') {
            ...
            const delimeters: string[] = this.utilService.getDateFormatDelimeters(df);
            const dateValue: IMyDateFormat[] = this.utilService.getDateValue(sd, df, delimeters);

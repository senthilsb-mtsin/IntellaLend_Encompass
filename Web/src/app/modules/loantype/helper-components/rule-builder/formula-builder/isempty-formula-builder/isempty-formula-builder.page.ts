    ...
    ngOnInit(): void {
        ...
        this._subscriptions.push(this.rulesFrmGrp.valueChanges.subscribe((form) => {
            ...
            myForm.isEmptyRule.forEach(elt => {
                if (elt.EmptyDocTypes !== '' && typeof (elt.EmptyDocFieldTypes) === 'string' && elt.EmptyDocFieldTypes !== '') {
                    str += '[' + elt.EmptyDocTypes + '.' + elt.EmptyDocFieldTypes + ']';
                } else if (elt.EmptyDocTypes !== '' && elt.EmptyDocFieldTypes !== '' && Array.isArray(elt.EmptyDocFieldTypes) && elt.EmptyDocFieldTypes.length > 0) {

    ...
    public class EvaluationVisitor : LogicalExpressionVisitor
    {
        ...
        public int CompareUsingMostPreciseType(object a, object b)
        {
            try
            {
                ...
                //Senthil update for caseinsensitivecompare   return Comparer.Default.Compare(Convert.ChangeType(a, mpt), Convert.ChangeType(b, mpt));
                return CaseInsensitiveComparer.Default.Compare(Convert.ChangeType(a, mpt), Convert.ChangeType(b, mpt));
            }
            catch (Exception ex)
            {
        ...
        public override void Visit(Identifier parameter)
        {
            if (Parameters.ContainsKey(parameter.Name))
            {
                ...
                if (Parameters[parameter.Name] is Expression)
                {
                    ...
                    Result = ((Expression)Parameters[parameter.Name]).Evaluate().Result;
                }
                else
            ...
            else
            {
                ...
                if (!args.HasResult)
                {
                    if (parameter.Name.Split('.').Length == 2)
                    {
                        throw new ArgumentException("DocumentType/Field not found for '" + parameter.Name + "'");
                    }
                    else
